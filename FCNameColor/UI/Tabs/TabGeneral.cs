...
public class TabGeneral(Plugin plugin) : ConfigTab(plugin)
{
    public override void Draw(Func<bool> markDirty)
    {
        ...
        ImGuiUtils.AddCheckbox("Only color the FC tag", "This will only colour the FC tag instead of the entire name.",
        ...
        using (ImRaii.Table("###OptionsTable", 2, ImGuiTableFlags.SizingFixedFit))
        {
            ...
            ImGui.TableNextColumn();
            ImGuiUtils.AddCheckbox("Ignore friends", "Don't change the nameplates of friends.", Config.IgnoreFriends,
                value => Config.IgnoreFriends = value, markDirty);

            ImGui.TableNextColumn();
            ImGuiUtils.AddCheckbox("Include duties", "Will colour the entire names of FC members when inside a duty",
        ...
        using (ImRaii.PushIndent())
        {
            ...
            ImGuiUtils.AddCheckbox("Hide party members", "Whether party members are hidden", Config.HidePartyMembers,
